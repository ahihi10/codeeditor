...
const LanguageSelector = ({hasAccess} : {hasAccess: boolean}) => {
  ...
  const handleLanguageSelect = (langId: string) => {
    const allowedLanguages = ["javascript", "typescript", "ts", "python", "ruby", "java", "go", "rust", "c++", "c#", "swift"]; 
    
    // Kiểm tra nếu người dùng có quyền truy cập và ngôn ngữ được chọn có trong danh sách cho phép
    if (!hasAccess && !allowedLanguages.includes(langId)) return;
  ...
  return (
    <div className='relative' ref={dropdownRef}>
      ...
      <ChevronDownIcon
      className={`size-5 text-gray-400 transition-all duration-300 group-hover:text-gray-300
        ${isOpen ? "rotate-180" : ""}`
                  ...
                  className='relative group px-2'>
                    <button
                      ...
                      onClick={()=> handleLanguageSelect(lang.id)}>
                        ...
                        <div className="absolute inset-0 bg-gradient-to-r from-blue-500/5 to-purple-500/50 
                        rounded-lg
